...
Server.prototype._onClientHandshake = function(state, data) {
	...
	if (found) {
		...
		if (reply.identity == 'replica-member') {
			var replicaMember = new ReplicaMember.ReplicaMember(this,
			...
			if (this.role == MASTER) {
				replicaMember.role = SLAVE;
				replicaMember.synchronize(this.database, function(err) {
					replicaMember.state = READY;
				});
			} else if (reply.role == 'master') {
				replicaMember.role  = MASTER;
				replicaMember.state = READY;
			} else {
				replicaMember.role  = SLAVE;
			}
			...
			var client = new Client.Client(state.socket, state.id);
...
Server.prototype._onClientCommand = function(client, command, remainingSocketData) {
	...
	if (command.command == 'add') {
		...
		client.socket.pause();
		database.get(command.group, command.timestamp / 60 / 60 / 24, command.offset,
			function(err, data)
		{
			if (err == 'not-found') {
...
Server.prototype._onClientAddData = function(client, data) {
	...
	if (done) {
		...
		this.database.add(group, timestamp / 60 / 60 / 24,
...
Server.prototype._logClientConnectionActivity = function(client, message) {
	var args = ["[Client %d] " + message, client.id];
