...
Database.prototype.add = function(groupName, timestamp, buffers, checksumBuffer, callback) {
	...
	this.findOrCreateTimeEntry(groupName, timestamp / 60 / 60 / 24, function(err, timeEntry) {
		...
		totalSizeBuf = BufferUtils.uintToBuffer(totalSize);
		
		prevOffset = timeEntry.dataFileSize;
		
		flushed = timeEntry.stream.write(DATA_FILE_HEADER);
		...
		flushed = timeEntry.stream.write(totalSizeBuf);
		timeEntry.dataFileSize += totalSizeBuf.length;
		console.assert(!flushed, "Code assumes that file I/O is never flushed immediately");
		...
		timeEntry.stream.once('drain', function() {
			callback(undefined, prevOffset);
		});
		timeEntry.stream.once('error', function(err) {
			callback(err);
		});
	})
}
