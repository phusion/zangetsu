...
Toc.prototype.findOrCreateGroup = function(groupName, callback) {
...
Toc.prototype.findOrCreateTimeEntry = function(groupName, dayTimestamp, callback) {
	if (!Group.validateGroupName(groupName)) {
		callback(new Error('Invalid group name'));
		return;
	}
	...
	if (!group) {
		this.groupCount++;
		group = this.groups[groupName] = new Group.Group(name,
			path.join(this.dbpath, groupName));
	}
	...
	if (!timeEntry) {
		...
		timeEntry = group.timeEntries[dayTimestamp] =
			new TimeEntry.TimeEntry(dayTimestamp,
				path.join(this.dbpath, groupName, dayTimestamp),
				0);
	}
