...
function Server(dbpath) {
	...
	this.clientCount = 0;
	this.lastClientId = 0;
	this.toc = new Toc(dbpath);
}

Server.prototype.listen = function(port, callback) {
	var self = this;
	this.toc.reload(function(err) {
		if (err) {
			callback(err);
		} else {
...
Server.prototype._onClientAddData = function(client, data) {
	...
	if (client.currentCommand.remaining < 0) {
		...
		function openCallback(groupName, timestamp, buffers, checksum, err, stream) {
			if (err) {
				disconnectClientWithError(client, "I/O error: " + err.message);
				return;
			}
			
			var timeEntry = this.toc.findOrCreateTimeEntry(groupName,
				timestamp / 60 / 60 / 24);
			var flushed = true;
			var offset = timeEntry.dataFileSize;
			var i;
			
			flushed = flushed && stream.write(DATA_FILE_HEADER);
			timeEntry.dataFileSize += DATA_FILE_HEADER.length;
			flushed = flushed && stream.write(checksum);
			timeEntry.dataFileSize += checksum.length;
			for (i = 0; i < buffers.length; i++) {
				flushed = flushed && stream.write(buffers[i]);
		...
		/* var timeEntry = this.toc.findOrCreateTimeEntry(
			client.currentCommand.group,
			client.currentCommand.timestamp / 60 / 60 / 24); */
		
		filename = this._determineFilename(
			client.currentCommand.group,
			client.currentCommand.timestamp);
		this._openFileWithCache(filename, openCallback.bind(this,
			client.currentCommand.group,
			client.currentCommand.timestamp,
			client.addBuffers,
			client.addChecksum.toBufer()));
		client.addBuffers = [];
...
Server.prototype._determineFilename = function(group, timestamp) {
	
}

Server.prototype._openFileWithCache = function(filename, callback) {
