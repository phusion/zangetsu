...
ShardRouterClient.prototype._handleCommand_results = function(command, callback) {
	var self = this;
	if (command.discard) {
		this.results = {};
		callback();
	...
	function getResults() {
		self.database.results(function(results) {
			if(self.connected()){
				self._writeJSON({status: 'ok', results: results});
				self.results = {};
				callback();
			}
		});
