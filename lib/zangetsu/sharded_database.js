...
Database.prototype.add = function(groupName, dayTimestamp, buffers, checksumBuffer, options, callback, handler) {
	var self = this;
	...
	var add = function() {
		...
		self.toc[groupName][dayTimestamp].shard.add(groupName, 3600 * 24 * dayTimestamp, opid, size, buffers, write_callback);
		// Add shard to list of shards we issued add requests to
		self.addingShards[self.toc[groupName][dayTimestamp].shard] = true;
	}
	...
	if(exists()) {
		...
		this.lock(groupName, dayTimestamp, function(gotLock) {
			if(gotLock) {
				// we've got the lock, lets make the file
				var shardList = [];
				for(shardId in self.shards) {
					shardList.push(self.shards[shardId]);
				}
				// pick a random shard
				var shard = shardList[Math.floor(Math.random() * shardList.length)];

				self.toc[groupName] = {};
				self.toc[groupName][dayTimestamp] = {shard: shard};
				add();	
			} else {
			   // we don't own the lock, someone else got before us	
				if(exists()) {
					...
					self.add(groupName, dayTimestamp, buffers, checksumBuffer, callback);
				}
			}
		});
	}
...
Database.prototype.nextOpid() {
	this.previousOpid++;
	return this.hostname + '/' + this.previousOpid;
}

// Fetch the results
Database.prototype.results = function(callback, handler) {
	var self = this;
	// call results on all relevant shards
	// addingShards.each(&:results)
	...
	var processResults = function(results) {
		for(opid in results) {
			var thisHandler = self.handlers[opid];
			if (thisHandler != undefined) {
				if(self.results[thisHandler] == undefined) { self.results[thisHandler] = [];}
				self.results[thisHandler].append(results[opid]);
			}
		}
	}

	// when done: return relevant opids
	// and remove relevant opids from buffered opids
	var done = function() {
		var results = self.results[handler];
...
Database.prototype.lock = function(group, dayTimestamp, callback) {
	...
	if(this.lockTable[key] != undefined) {
		...
		this.receiveLock(this.hostname, group, dayTimestamp);
	}
}
