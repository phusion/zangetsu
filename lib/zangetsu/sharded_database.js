...
Database.prototype.add = function(groupName, dayTimestamp, buffers, checksumBuffer, callback) {
	...
	var add = function() {
		self.toc[groupName][dayTimestamp].shard.add(groupName, 3600 * 24 * dayTimestamp, 1, 11, buffers, callback);
	}
	...
	if(exists()) {
		...
		this.lock(groupName, dayTimestamp, function(gotLock) {
			if(gotLock) {
				//we hebben de lock, make the file:
				var shardList = [];
			   ...
			   //	we hebben de lock niet: iemand anders ging ons voor
				if(exists()) {
					// de file bestaat nu gewoon, we kunnen data adden
					add();
				} else {
					// de file is gedelete door onze voorganger, het commando moet overnieuw
