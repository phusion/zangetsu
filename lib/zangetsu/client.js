...
function Client(server, socket, input, id) {
...
Client.prototype._onClientCommand = function(command) {
	...
	if (command.command == 'add') {
		...
		function writeResults() {
			writeMessage(self.socket, { status: 'ok', results: self.results });
			self.results = {};
		}
		...
		this.database.get(command.group, dayTimestamp, command.offset, function(err, data) {
			if (!self.connected()) {
				...
				writeMessage(self.socket, { status: 'not-found' });
			} else if (err) {
				self._disconnectWithError(self, "Cannot get requested data: " +
					err.message);
				return;
			} else {
				writeMessage(self.socket, { status: 'ok', size: data.length });
		...
		function deleted(err) {
			...
			if (!self.connected()) {
				...
				writeMessage(self.socket, { status: 'error',
					message: err.message });
			} else {
				writeMessage(self.socket, { status: 'ok' });
			}
			if (paused) {
				self.input.resume();
			}
		...
		writeMessage(this.socket, { status: 'ok' });
		return false;
