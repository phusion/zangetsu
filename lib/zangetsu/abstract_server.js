...
var ShardRouterClient = require('./shard_router_client.js');
...
exports._handleHandshakeReply = function(state, reply) {
	...
	case Constants.ROLE_SHARD_ROUTER:
		mode = CommandHandler.MODE_NORMAL;
		break;
	case Constants.ROLE_SLAVE:
		mode = CommandHandler.MODE_SLAVE;
		break;
	default:
	...
	if (reply.identity == 'replica-slave') {
		...
		var client;
		if(this.role == Constants.ROLE_SHARD_ROUTER) {
			client = new ShardRouterClient.ShardRouterClient(this, state.socket,
					state.input, state.output, mode, state.id);
		} else {
			client = new Client.Client(this, state.socket, state.input,
				state.output, mode, state.id);
		}
