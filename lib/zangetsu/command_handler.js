...
CommandHandler.prototype._startClosing = function() {
	var self = this;
	console.assert(this.state == WAITING_FOR_COMMAND || this.state == PROCESSING_COMMAND);
	this.state = CLOSING;
	if (this.socket.bufferSize == 0) {
		this.state = DISCONNECTED;
		self.socket.destroy();
...
CommandHandler.prototype.readNextCommand = function() {
	console.assert(this.state == NOT_STARTED || this.state == PROCESSING_COMMAND);
	this.state = WAITING_FOR_COMMAND;
	...
	readJsonObject(this.input, function(err, object) {
		if (err) {
			...
			if (getType(object.command) != 'string') {
				...
				var handler = self['_handleCommand_' + object.command];
				if (handler) {
					handler.call(self, object);
				} else {
					self._disconnectWithError("Unknown command " + object.command);
				}
			}
		}
	});
...
CommandHandler.prototype._handleCommand_add = function(command) {
...
CommandHandler.prototype._handleCommand_results = function(command) {
	var self = this;
...
CommandHandler.prototype._handleCommand_get = function(command) {
	var self = this;
...
CommandHandler.prototype._handleCommand_remove = function(command) {
	if (getType(command.group) != 'string') {
		this._disconnectWithError("Expecting a 'group' string field");
...
CommandHandler.prototype._handleCommand_removeOne = function(command) {
	if (getType(command.group) != 'string') {
		this._disconnectWithError("Expecting a 'group' string field");
...
CommandHandler.prototype._handleCommand_getToc = function(command) {
	this._writeJSON(this.database.toTocFormat());
	this.readNextCommand();
}

CommandHandler.prototype._handleCommand_ping = function(command) {
	var self = this;
