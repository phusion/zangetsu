...
Server.prototype.startAsReplicaMember = function(host, port, publicHostName, masterHost, masterPort) {
	this._startAsReplicaMember(publicHostName, masterHost, masterPort, function(server) {
		server.listen(port, host);
	});
}

Server.prototype.startAsReplicaMemberWithFD = function(fd, publicHostName, masterHost, masterPort) {
	this._startAsReplicaMember(publicHostName, masterHost, masterPort, function(server) {
		server.listenFD(fd);
	});
}

Server.prototype._startAsReplicaMember = function(publicHostName, masterHost, masterPort, initialize) {
	console.assert(this.server === undefined);
	
	var self = this;
	var memberSocket, input, output;
	var commandBuffer;
	var jsonHandler;
	...
	function connectToMaster(timeout) {
		memberSocket  = new net.Socket();
		commandBuffer = '';
		setTimeout(function() {
			memberSocket = new net.Socket();
			input        = new SocketInputWrapper.SocketInputWrapper(memberSocket);
			output       = new SocketOutputWrapper.SocketOutputWrapper(memberSocket);
			input.onData = onData;
			memberSocket.on('error', function() {
				if (jsonHandler) {
					var handler = jsonHandler;
					jsonHandler = undefined;
					handler(new Error('Connection with master closed unexpectedly'));
			...
			memberSocket.connect(masterPort, masterHost, onConnect);
		}, timeout);
	}
	
	function disconnectFromMaster() {
		memberSocket.destroySoon();
		memberSocket  = undefined;
		input         = undefined;
		output        = undefined;
		commandBuffer = '';
		jsonHandler   = undefined;
	...
	function onConnect(err) {
		...
		receiveJSON(function(err, object) {
			if (err) {
	...
	function connectedToMaster(masterHostName, masterPort) {
		receiveNextReplicationCommand();
	}
	
	function receiveNextReplicationCommand() {
		receiveJSON(function(err, input) {
			if (err) {
				return reconnectToMaster("Error receiving replication command from master");
			}
			
			switch (input.command) {
			case 'getToc':
				output.writeJSON(self.getToc(), sentReplicationCommandReply);
				break;
			case 'add':
				output.writeJSON({ status: 'ok' }, sentReplicationCommandReply);
				break;
			case 'removeOne':
				output.writeJSON({ status: 'ok' }, sentReplicationCommandReply);
				break;
	...
	function sentReplicationCommandReply(err) {
		if (err) {
			reconnectToMaster("Error sending reply to master");
		} else {
			receiveNextReplicationCommand();
		}
	}
