 ...
 *
 * If a premature EOF occurs (that is, if not EXACTLY 'size' bytes
 * are read) then the callback is called with
 ...
 * malformed) then:
 * - the callback is called with an error, but the
 *   SocketInputWrapper's onError callback is NOT called.
 * - the error object will not have an 'isIoError' attribute.
 * - a 'jsonParseError' event will be emitted on the SocketInputWrapper's
 *   underlying socket, after calling the callback.
 *
...
exports.readJsonObject = function(input, callback) {
	...
	input.onData = function(data) {
		...
		if (found) {
			...
			if (error) {
				input.socket.emit('jsonParseError', e);
			}
		}
		return consumed;
	}
