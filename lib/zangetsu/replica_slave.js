...
ReplicaSlave.prototype._continueReplication = function(toc) {
	...
	function fillByStreamingDataFile(details) {
		details.timeEntry.get(details.timeEntryOnReplica.size, function(err) {
			if (!self.connected()) {
				return;
			}
			if (err && err.code != 'ERR_MARKED_CORRUPTED') {
			...
			function(err, buf, rawSize, continueReading, stop)
		{
			...
			if (!self.connected()) {
				...
				self._write(buf, function(err) {
					dataFileRecordSent(err, details, progress,
						continueReading, stop);
				});
	...
	function dataFileRecordSent(err, details, progress, continueReading, stop) {
		if (!self.connected() || err) {
			return stop();
		}
				...
				if (err || !self.connected()) {
					...
					self._readJsonObject(function(object) {
						handleCheckResultAfterSendingDataFileRecord(
							object, details, continueReading, stop);
					});
				}
			});
		} else {
	...
	function handleCheckResultAfterSendingDataFileRecord(reply, details, continueReading, stop) {
