...
ReplicaSlave.prototype._continueReplication = function(toc) {
			...
			function(err, buf, rawSize, continueReading, stop)
		{
			...
			if (!self.connected()) {
				...
				self._write(buf, function(err) {
					dataFileRecordWritten(err, details, progress,
	...
	function dataFileRecordWritten(err, details, progress, continueReading, stop) {
		if (!self.connected() || err) {
			return stop();
		}
		...
		if (resultCheckCounter >= self.resultCheckThreshold) {
			...
			self._writeJSON({ command: 'results' }, function(err) {
				if (err || !self.connected()) {
					...
					self._readJsonObject(function(object) {
						handleCheckResultAfterWritingDataFileRecord(
	...
	function handleCheckResultAfterWritingDataFileRecord(reply, details, continueReading, stop) {
