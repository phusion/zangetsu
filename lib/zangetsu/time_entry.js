...
TimeEntry.prototype.add = function(buffers, checksumBuffer, callback) {
	...
	function written(err) {
		...
		if (err) {
			...
			if (self.database) {
				self.database.registerDataGrowth(self, totalSize);
			}
			var allBuffers = [header];
			for (i = 0; i < buffers.length; i++) {
				allBuffers.push(buffers[i]);
			}
			allBuffers.push(footer);
			callback(undefined, prevOffset, totalSize, allBuffers);
		}
	}
	...
	IOUtils.uintToBuffer(totalDataSize, footer, CRC32_BINARY_SIZE);
