...
TimeEntry.prototype.add = function(buffers, checksumBuffer, callback) {
	...
	function written(err) {
		...
		if (err) {
			...
			NativeSupport.posix_fadvise(self.stream.fd, prevOffset,
				HEADER_SIZE + totalSize, NativeSupport.POSIX_FADV_DONTNEED || 0,
				flushed);
		}
	}
	
	function flushed(err) {
		console.assert(!self.closed);
		self._decWriteOperations();
		self._verifyInvariants();
		if (err && err.code != 'ENOSYS') {
	...
	this.dataFileSize += HEADER_SIZE + totalSize;
	NativeSupport.posix_fallocate(this.stream.fd, this.dataFileSize,
		header.length + totalSize, spaceAllocated);
}
