...
TimeEntry.prototype.quickRepair = function() {
	console.assert(!this.isClosed());
	console.assert(this.readOperations == 0);
	console.assert(this.writeOperations == 0);
	console.assert(this.writtenSize == this.dataFileSize);
	...
	var self = this;
	var done = false;
	var endOffset = this.dataFileSize;
	var footer, header, sizeFieldTrusted, result;

	/* We start scanning at the last record and move backwards until
	 * we either encounter a valid record, or until we've reached
	 * an offset before which no record can exist.
	 */

	// These variables point to the offset and size of the
	// record that we're currently examening.
	var offset, size;
	
	while (!done && endOffset >= HEADER_SIZE + FOOTER_SIZE) {
		footer = readFooter(endOffset - FOOTER_SIZE);

		if (!bufferEquals(footer.magic, FOOTER_MAGIC)) {
			offset = file.dataFileSize - FOOTER_SIZE - footer.size - HEADER_SIZE;
			size   = HEADER_SIZE + footer.size + FOOTER_SIZE;
			header = readHeader(offset);
			sizeFieldTrusted =
				header.size == footer.size &&
				header.size >= HEADER_SIZE + FOOTER_SIZE;
		} else {
			sizeFieldTrusted = false;
		}

		if (isLastRecordInFile()) {
			if (!sizeFieldTrusted) {
				result = searchBackwardsForHeaderMagicOrFooterMagic(
					endOffset - FOOTER_MAGIC.length);
				if (!result) {
					// Looks like the entire file is corrupted.
					truncateFile(0);
					done = true;
				} else if (result.isFooterMagic) {
					offset = result.offset + FOOTER_MAGIC_SIZE;
				} else {
					console.assert(result.isHeaderMagic);
			...
			if (!done) {
				if (size < HEADER_SIZE + FOOTER_SIZE) {
					// This record is too small for us to repair,
					// so remove it.
					truncateFile(offset);
				} else if (checkRecord(offset, size)) {
					done = true;
				} else {
					markCorrupted(offset, size);
				}
			}
			...
			if (!sizeFieldTrusted) {
				result = searchBackwardsForHeaderMagicOrFooterMagic(
					endOffset - FOOTER_MAGIC.length, true);
				if (!result) {
					// Looks like everything from the beginning of
					// the file until endOffset is corrupted.
					markCorrupted(0, endOffset);
					done = true;
				} else if (result.isFooterMagic) {
					offset = result.offset + FOOTER_MAGIC_SIZE;
				} else {
			...
			if (!done) {
				console.assert(size >= HEADER_SIZE + FOOTER_SIZE);
				if (checkRecord(offset, size)) {
					done = true;
				} else {
					markCorrupted(offset, size);
				}
	...
	if (!done && endOffset != 0 && endOffset < HEADER_SIZE + FOOTER_SIZE) {
		/* For some reason the first record is truncated beyond the minimum size
		 * while all the records after it (if any) are not. The first record should
		 * be merged with the next record and marked corrupted. If there is no next
		 * record then the file should be truncated to 0 bytes.
		 */
		if (endOffset == file.dataFileSize) {
			truncateFile(0);
		} else {
			header = readHeader(endOffset);
			markCorrupted(0, endOffset + HEADER_SIZE + header.size + FOOTER_SIZE);
		}
	}

	function readFooter(offset) {
		var buf = new Buffer(FOOTER_SIZE);
		var bytesRead = fs.readSync(this.stream.fd, buf, 0, FOOTER_SIZE, offset);
		console.assert(bytesRead == FOOTER_SIZE);
		return {
			crc  : buf.slice(0, CRC32_BINARY_SIZE),
			size : buf.readUInt32BE(CRC32_BINARY_SIZE + RESERVED_DATA.length),
			magic: buf.slice(CRC32_BINARY_SIZE + RESERVED_DATA.length + SIZE_FIELD_SIZE)
		}
	}

	function readHeader(offset) {
		var buf = new Buffer(HEADER_SIZE);
		var bytesRead = fs.readSync(this.stream.fd, buf, 0, HEADER_SIZE, offset);
		console.assert(bytesRead == HEADER_SIZE);
 		return {
 			magic: buf.slice(0, HEADER_MAGIC.length),
 			size : buf.readUint32BE(HEADER_MAGIC.length),
 			flags: buf.readUInt8(HEADER_MAGIC.length + SIZE_FIELD_SIZE)
 		}
	}
	...
	function isLastRecordInFile() {
		return endOffset == self.dataFileSize;
	}

	function searchBackwardsForHeaderMagicOrFooterMagic(offset, recordSizeMustBeLargerThanMimimum) {
		console.error("not yet implemented");
		process.exit(1);
	}

	function truncateFile(size) {
		fs.truncateSync(self.stream.fd, size);
		self.dataFileSize = size;
		self.writtenSize = size;
	}

	function checkRecord(offset, size) {
		var record = new Buffer(size);
		var bytesRead = fs.readSync(self.stream.fd, record, 0, size, offset);
		console.assert(bytesRead == size);

		var dataSize = size - HEADER_MAGIC - FOOTER_MAGIC;

		if (!bufferEquals(record.slice(0, HEADER_MAGIC.length), HEADER_MAGIC)) {
			return false;
		}
		if (record.readUInt32BE(HEADER_MAGIC.length) != dataSize) {
			return false;
		}
		if (record.readUInt32BE(HEADER_SIZE + dataSize) !=
			CRC32.calculate(record.slice(HEADER_SIZE, HEADER_SIZE + dataSize)))
		{
			return false;
		}
		if (record.readUInt32BE(HEADER_SIZE + dataSize + CRC32_BINARY_SIZE +
			RESERVED_DATA.length) != dataSize)
		{
			return false;
		}
		if (!bufferEquals(record.slice(HEADER_SIZE + dataSize + CRC32_BINARY_SIZE +
			RESERVED_DATA.length + SIZE_FIELD_SIZE), FOOTER_MAGIC))
		{
			return false;
		}
		return true;
	...
	function markCorrupted(offset, size) {
		var record = new Buffer(size);
		var bytesRead = fs.readSync(self.stream.fd, record, 0, size, offset);
		console.assert(bytesRead == size);
		var dataSize = size - HEADER_MAGIC - FOOTER_MAGIC;

		HEADER_MAGIC.copy(record);
		record.writeUInt32BE(dataSize, HEADER_MAGIC.length);
		record.writeUInt8(CORRUPTION_FLAG, HEADER_MAGIC.length + SIZE_FIELD_SIZE);
		record.writeUInt32BE(CRC32.calculate(record.slice(HEADER_SIZE, HEADER_SIZE + dataSize)),
			HEADER_SIZE + dataSize);
		record.fill(0, HEADER_SIZE + dataSize + CRC32_BINARY_SIZE,
			HEADER_SIZE + dataSize + CRC32_BINARY_SIZE + RESERVED_DATA.length);
		record.writeUInt32BE(dataSize, HEADER_SIZE + dataSize +
			CRC32_BINARY_SIZE + RESERVED_DATA.length);
		FOOTER_MAGIC.copy(record, HEADER_SIZE + dataSize + CRC32_BINARY_SIZE +
			RESERVED_DATA.length + SIZE_FIELD_SIZE);
		
		fs.writeSync(self.stream.fd, record, 0, size, offset);
	}
}
