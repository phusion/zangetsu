...
Router.prototype.addShardRouter = function(description) {
	var server = new ShardRouterConnection(description);
...
Router.prototype.giveLock = function(identifier, group, dayTimestamp) {
	...
	this.whenWorkingFinished(function() {
		self.shardRouters[identifier].giveLock(key);
	});
}
	...
	if(this.isLocked(group, dayTimestamp)) {
		...
		for(identifier in this.shardRouters) {
			this.shardRouters[identifier].lock(group, dayTimestamp,
				function(result) {
					if(result.status == 'ok') {
						self.receiveLock(identifier, group, dayTimestamp);
					}
				});
		}
		this.whenWorkingFinished(function() {
...
Router.prototype.listLocks = function(identifier) {
	...
	this.shardRouters[identifier].replyLocks(locks);
}
