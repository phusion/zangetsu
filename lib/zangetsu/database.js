...
Database.prototype.add = function(groupName, dayTimestamp, buffers, checksumBuffer, callback) {
	...
	function done(err, offset, size, buffers) {
	...
	function reallyAdd() {
		timeEntry.add(buffers, checksumBuffer, function(err, offset, size, buffers) {
			...
			var listeners = self.listeners('add');
			if (listeners.length > 0 && !err) {
				var counter = listeners.length;
				self.emit('add', groupName, dayTimestamp, offset, size, buffers, function() {
					console.assert(counter > 0);
					counter--;
					if (counter == 0) {
						done(err, offset, size, buffers);
					}
				});
			} else {
				done(err, offset, size, buffers);
	...
	if (this.state == LOCKING || this.state == LOCKED) {
		this.waitingAddOperations.push(perform);
	} else {
		perform();
	}
}
