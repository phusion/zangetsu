...
Database.prototype.syncAndEvict = function() {
	...
	var i, counter = 0, totalUnevicted = 0;
	...
	function doSyncAndEvinct(timeEntry) {
		timeEntry.sync(function(err) {
			if (timeEntry.isClosed() || err) {
				decCounter();
			} else {
				timeEntry.flush(decCounter);
			}
		});
	}
	...
	for (i = 0; i < timeEntries.length; i++) {
		if (!timeEntries[i].isClosed()) {
			totalUnevicted += timeEntries[i].unevictedSize;
			counter++;
		}
	}
	for (i = 0; i < timeEntries.length; i++) {
	...
	} else if (totalUnevicted < 1024 * 1024) {
		console.log("Evicting %s KB over %d entries...",
			(totalUnevicted / 1024).toFixed(2),
			timeEntries.length);
	} else {
		console.log("Evicting %s MB over %d entries...",
			(totalUnevicted / 1024 / 1024).toFixed(2),
			timeEntries.length);
	}
}
