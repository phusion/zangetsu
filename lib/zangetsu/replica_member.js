...
ReplicaMember.prototype.initialize = function() {
	var reply = { status: 'ok' };
	var self  = this;
	if (this.server.role == Server.MASTER) {
		this.role = SLAVE;
		this._writeJSON({
			status   : 'ok',
			your_role: 'slave',
		...
		console.assert(this.server.role == Server.SLAVE);
		this.role = SLAVE;
		this._writeJSON({
			status     : 'not-master',
...
ReplicaMember.prototype._continueReplication = function(toc) {
				...
				if (localTimeEntry) {
					...
					if (timeEntryOnReplica
					 && localTimeEntry.writtenSize < timeEntryOnReplica.size) {
						this._logDebug("Scheduling refill: %s/%d (%s)",
							groupName, dst,
	...
	function sentPieceOfDataFile(err, details, buf, continueReading, stop) {
		if (err || !self.connected()) {
			stop();
			return;
		}
