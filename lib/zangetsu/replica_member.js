...
ReplicaMember.prototype._continueReplication = function(toc) {
	...
	function fillNext() {
		...
		if (details) {
			...
			self._logDebug("Fill next:", debugDetails);
			...
			if (details.buffers) {
				...
				fillBySendingBuffers(timeEntryOnReplica,
					details.groupName,
					details.dayTimestamp,
					details.buffers);
			} else {
	...
	function fillBySendingBuffers(timeEntryOnReplica, groupName, dayTimestamp, buffers) {
		console.assert(self.state == READY);
			...
			for (i = 0; i < buffers.length; i++) {
				if (i == buffers.length - 1) {
					self._write(buffers[i], function(err) {
						sentAllBuffers(err, timeEntryOnReplica, totalSize);
					});
				} else {
	...
	function sentAllBuffers(err, timeEntryOnReplica, totalSize) {
