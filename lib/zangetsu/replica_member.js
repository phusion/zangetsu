...
ReplicaMember.prototype._continueReplication = function(toc) {
	...
	function fillBySendingBuffers(groupName, dayTimestamp, buffers) {
		...
		if (buffer) {
			...
			self._write(buffer, function(err) {
				sentOneBuffer(err, buffer, groupName,
	...
	function sentOneBuffer(err, buffer, groupName, dayTimestamp, remainingBuffers) {
		...
		self.onJSON = function(reply) {
			...
			if (reply.status == 'ok') {
				...
				timeEntryOnReplica.size += buffer.length;
				fillBySendingBuffers(groupName, dayTimestamp, remainingBuffers);
			} else {
				self._disconnect();
			}
		}
